...
<div id="loginBox">
    ...
    <h1 id="logo"><a href="index.php">
        ...
        <img src="logo.php?login" alt="osTicket :: <?= __('Staff Control Panel');?>" />
    </a></h1>
    <h3 id="login-message"><?= Format::htmlchars($msg); ?></h3>
    <div class="banner"><small><?= ($content) ? Format::display($content->getLocalBody()) : ''; ?></small></div>
    <form action="login.php" method="post" id="login" onsubmit="attemptLoginAjax(event)">
        ...
        <input type="text" name="userid" id="name" value="<?php
            echo $info['userid']; ?>" placeholder="<?= __('Email or Username'); ?>"
            autofocus autocorrect="off" autocapitalize="off">
        <input type="password" name="passwd" id="pass" placeholder="<?= __('Password'); ?>" autocorrect="off" autocapitalize="off">
            <h3 style="display:inline"><a id="reset-link" class="<?php
                if (!$show_reset || !$cfg->allowPasswordReset()) echo 'hidden';
                ?>" href="pwreset.php"><?= __('Forgot My Password'); ?></a></h3>
...
<div class="external-auth"><?php $bk->renderExternalLink(); ?></div><?php
    ...
    <div id="company">
        <div class="content">
            <?= __('Copyright'); ?> &copy; <?= Format::htmlchars($ost->company) ?: date('Y'); ?>
        </div>
    </div>
</div>
<div id="poweredBy"><?= __('Powered by'); ?>
    ...
    <![endif]-->
    <script type="text/javascript" src="<?= ROOT_PATH; ?>js/jquery-ui-1.12.1.custom.min.js?f1e9e88"></script>
</body>
</html>
